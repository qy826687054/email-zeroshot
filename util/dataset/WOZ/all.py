...
class DatasetTool(object):
    ...
    def get_set(file, trans = False, idx_dict = None, lan_id = None):
        raw = util.data.Reader.read_json(file)
        fix = {'centre': 'center', 'areas': 'area', 'phone number': 'number', "price range": "price"}
        turns = []
        for dia in raw:
            for turn in dia["dialogue"]:
                ...
                if not trans:
                    ...
                    turn["system_acts"] = [idx_dict.src2tgt[lan_id]["request"][-1] + " " + x if isinstance(x, str) else \
                        idx_dict.src2tgt[lan_id]["inform"][-1] + " " + x[0] + " = " + x[1] for x in turn["system_acts"]]
                turn["system_acts"].append("")
                turns.append(turn)
        dataset = util.convert.Common.to_args(turns, True)
        return dataset

    def get_idx_dict(word_dict, file, args, ontology_src, ontology_tgt):
        raw = util.data.Delexicalizer.remove_linefeed(util.data.Reader.read_raw(file))
        word_dict.src2tgt.append({})
        ...
        for line in raw:
            try:
                ...
                src, tgt = line.split(" ")
            if src not in word_dict.src2tgt[-1]:
                word_dict.src2tgt[-1][src] = [tgt]
            else:
                word_dict.src2tgt[-1][src].append(tgt)
        for slot in ontology_src.slots:
            for (x, y) in zip(ontology_src.values[slot], ontology_tgt.values[slot]):
                if x not in word_dict.src2tgt[-1]:
                    word_dict.src2tgt[-1][x] = [y]
                else:
                    word_dict.src2tgt[-1][x].append(y)
    ...
    def get(args):
        ...
        test_it_file = os.path.join(args.dir.dataset, "WOZ/woz_it", "test.json")
        test_de_file = os.path.join(args.dir.dataset, "WOZ/woz_de", "test.json")
        ontology_src_file = os.path.join(args.dir.dataset, args.dataset.src, "ontology.json")
        ontology_tgt_it_file = os.path.join(args.dir.dataset, "WOZ/woz_it", "ontology.json")
        ontology_tgt_de_file = os.path.join(args.dir.dataset, "WOZ/woz_de", "ontology.json")
        dict_it_file = os.path.join(args.dir.dataset, "Panlex/dict/it2.txt")
        dict_de_file = os.path.join(args.dir.dataset, "Panlex/dict/de2.txt")
        train = DatasetTool.get_set(train_file)
        dev = DatasetTool.get_set(dev_file)
        word_dict = util.convert.Common.to_args({"src2tgt": []})
        ontology_src = DatasetTool.get_ontology(ontology_src_file, args.dataset.dontcare_src)
        ontology_it_tgt = DatasetTool.get_ontology(ontology_tgt_it_file, args.dataset.dontcare_tgt_it)
        ontology_de_tgt = DatasetTool.get_ontology(ontology_tgt_de_file, args.dataset.dontcare_tgt_de)
        DatasetTool.get_idx_dict(word_dict, dict_it_file, args, ontology_src, ontology_it_tgt)
        DatasetTool.get_idx_dict(word_dict, dict_de_file, args, ontology_src, ontology_de_tgt)
        test_it = DatasetTool.get_set(test_it_file, True, word_dict, 0)
        test_de = DatasetTool.get_set(test_de_file, True, word_dict, 1)
        return train, dev, (test_it, test_de), (ontology_src, ontology_it_tgt, ontology_de_tgt), word_dict, None
